...
def generate_graphical_representation_for_repositories_relationship(owner, submodule_relationship):
...
def main():
    ...
    parser.add_argument('-o', '--owner', nargs='?', required=True, default=None, type=str)
    ...
    matches = GhCliAdapter.search_code("", args.owner, matching_filename=False, filename=".gitmodules")
    ...
    for match in matches:
        ...
        GhCliAdapter.download_file(owner, repository, match["path"], downloaded_gitmodules_location)

        if current_repository_with_owner not in submodule_relationship:
            submodule_relationship[current_repository_with_owner] = Repository(owner, repository)
        ...
        with open(downloaded_gitmodules_location) as current_gitmodule:
            ...
            for match in matches:
                ...
                submodule_relationship[current_repository_with_owner].add_submodule (submodule)
                if submodule not in submodule_relationship:
                    submodule_relationship[submodule] = Repository(owner, repo)
                submodule_relationship[submodule].reference_this_as_submodule(current_repository_with_owner)
        os.remove(downloaded_gitmodules_location)

    # create a graph of relationship
    generate_graphical_representation_for_repositories_relationship(args.owner, submodule_relationship)
